    ...
    public override void _PhysicsProcess(double delta)
    {
        try
        {
            if (
                ...
                || Universe.PlayerPivot == null
                || Universe.CameraArm == null
            )
            {
                return;
            }
            ...
            switch (face)
            {
                case Face.Top:
                    ...
                    y = Mathf.FloorToInt(
                        (1f - ((cubemap.Z + 1f) / 2f)) * Universe.Planet.Resolution
                    );
                    break;
                case Face.Bottom:
                    x = Mathf.FloorToInt(
                        (1f - ((cubemap.X + 1f) / 2f)) * Universe.Planet.Resolution
                    );
                    y = Mathf.FloorToInt(
                        (1f - ((cubemap.Z + 1f) / 2f)) * Universe.Planet.Resolution
                    );
                ...
                case Face.Left:
                    ...
                    y = Mathf.FloorToInt(
                        (1f - ((cubemap.Y + 1f) / 2f)) * Universe.Planet.Resolution
                    );
                    break;
                case Face.Right:
            ...
            float maxElevation = new float[]
            {
                ...
                x < xLength - 1 && y < yLength - 1
                    ? terrainFace.Elevations[x + 1, y + 1].scaled
                    : 0,
                ...
                x < xLength - 3 && y < yLength - 3
                    ? terrainFace.Elevations[x + 3, y + 3].scaled
                    : 0,
            ...
            else
            {
                ...
                if (
                    ...
                    && _targetHeight > maxElevation
                )
                {
                    _targetHeight -=
                        Universe.Planet.Gravity * (float)delta * (150f + _fallFrames * 5);
                }
